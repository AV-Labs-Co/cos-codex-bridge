...
export const schemas={
 bridge_projects:z.object({action:z.enum(['list','create','register','inspect']).default('list'),project:z.string().optional(),parent:z.string().optional(),name:z.string().regex(/^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$/).optional()}).strict(),
 bridge_sessions:z.object({action:z.enum(['find','read']).default('find'),project:z.string().optional(),query:z.string().max(200).optional(),cursor:z.string().max(2048).optional(),threadId:z.string().max(200).optional(),includeOutput:z.boolean().default(false),includeQueue:z.boolean().default(false)}).strict(),
 bridge_submit:z.object({onBusy:z.enum(['reject','queue']).default('reject'),delivery:z.enum(['direct','desktop-queue']).default('direct'),acceptDesktopPolicy:z.boolean().default(false),project:z.string(),threadId:z.string().max(200).optional(),prompt:z.string().optional(),promptFile:z.string().optional(),artifacts:z.array(z.string()).max(8).default([]),requestId:z.string().regex(/^[a-zA-Z0-9_-]{8,100}$/)}).strict(),
...
export class Bridge {
 ...
 async projects(args:any){
  if(['register','inspect'].includes(args.action)){if(!args.project)throw new BridgeError('INVALID_ARGUMENT','register/inspect requires project.');const cwd=await project(this.config,args.project);const d=new Desktop(this.config,this.backend);return args.action==='register'?d.register(cwd):d.resolve(cwd);}
  if(args.action==='list'){const items=[];for(const [name,p] of Object.entries(this.config.projects)){try{items.push({name,path:await project(this.config,p)});}catch{items.push({name,unavailable:true});}}
   return {mode:this.config.mode,roots:this.config.roots,projects:items,registration:'Bridge aliases and workspace directories; not desktop sidebar registration.'};}
  ...
  return {path:await scoped(this.config,dest),created:true,desktopRegistered:false};
 }
 ...
 async submit(args:any){
  ...
  const fingerprint=hash(JSON.stringify({cwd,threadId:args.threadId??null,input,artifacts,onBusy:args.onBusy??'reject',delivery:args.delivery??'direct',acceptDesktopPolicy:args.acceptDesktopPolicy??false,mode:this.config.mode,sandbox:this.config.sandbox,model:this.config.model??null}));
  ...
  try{
   ...
   if(args.threadId){const t=await this.backend.read(args.threadId);if(await scoped(this.config,t.cwd)!==cwd)throw new BridgeError('PROJECT_MISMATCH','Thread belongs to a different project.');if(t.status?.type==='active'&&!queueRequested)throw new BridgeError('SESSION_BUSY','Codex reports this thread active.');}
   ...
   const now=new Date().toISOString();const receipt:Receipt={id,key:args.requestId,fingerprint,mode:this.config.mode,cwd,createdAt:now,updatedAt:now,state:'queued',promptBytes:bytes,promptSha256:hash(prompt),payloadSha256:hash(JSON.stringify(input)),artifacts,threadId:args.threadId};
   await this.store.atomic(this.store.file('payloads',id),{input,onBusy:args.onBusy,delivery:args.delivery,existingThread:!!args.threadId,configFingerprint:hash(JSON.stringify(this.config))});
   await this.store.save(receipt);
   const child=spawn(process.execPath,[fileURLToPath(new URL('./worker.js',import.meta.url)),this.config.configPath,id],{detached:true,stdio:'ignore',env:process.env});
   child.on('error',()=>{});child.unref();
   return {...receipt,note:'Queued, not completed. Poll bridge_receipt. Reuse requestId only for the identical submission.'};
 ...
 async receipt(id:string,includeOutput=true){let r=await this.store.get(id);await scoped(this.config,r.cwd);
  ...
  if(['busy','queued','preparing','accepted','steered','delivered','blocked'].includes(r.state)&&!alive(r.pid)&&Date.now()-Date.parse(r.updatedAt)>30000){
   return {...r,state:'uncertain',error:{code:'WORKER_LOST',message:'Worker is no longer available. No automatic retry. Read the saved Codex thread before deciding what to send next.'},...(includeOutput?{}:{output:undefined})};
