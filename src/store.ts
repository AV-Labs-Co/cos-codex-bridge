...
export type Receipt={id:string;key:string;fingerprint:string;mode:string;cwd:string;createdAt:string;updatedAt:string;state:'busy'|'blocked'|'steered'|'delivered'|'queued'|'preparing'|'accepted'|'completed'|'failed'|'cancelled'|'uncertain';promptBytes:number;promptSha256:string;payloadSha256:string;artifacts:Array<{path:string;bytes:number;sha256:string}>;threadId?:string;turnId?:string;pid?:number;error?:{code:string;message:string};output?:string;outputTruncated?:boolean;approvalsDenied?:number;pendingQuestions?:Array<{id:string;questions:any[]}>;desktop?:Record<string,unknown>;cancelRequested?:boolean;deliveryRoute?:'desktop-queue';queueMissingSince?:number;queuedSubmissionId?:string;queueClientMessageId?:string;queueHistory?:Array<{state:string;at:string;detail:string}>;ownerRecovery?:{state:"dispatching"|"acknowledged"|"blocked"|"uncertain";at:string;code?:string;turnId?:string;fieldVerified:false};queueObservation?:Record<string,unknown>};
export class Store {
