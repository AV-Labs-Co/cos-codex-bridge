...
if(args.includes('--help')||!args.includes('--root')){console.log('Install: node scripts/install.mjs --root /absolute/projects [--prefix /absolute/install-dir] [--demo] [--write] [--codex /absolute/codex] [--claude /absolute/claude]\nNo download or credential collection. Default sandbox: read-only. --write permits project writes. Never use / or your entire home.');process.exit(args.includes('--help')?0:1);}
try{
...
const config={version:1,mode:args.includes('--demo')?'demo':'codex',roots:[root],projects:{workspace:root},sandbox:args.includes('--write')?'workspace-write':'read-only',stateDir:path.join(prefix,'state'),codexBinary:args.includes('--codex')?value('--codex'):'codex',claudeBinary:args.includes('--claude')?value('--claude'):'claude'};
