...
The same installer is included in the [public npm package](https://www.npmjs.com/package/cos-codex-bridge). For Codex only, install `cos-codex-bridge@0.1.4`; for the Claude Code CLI preview, install `cos-codex-bridge@0.2.0-beta.1`. Put the package in a dedicated folder, then run `node <package-folder>/node_modules/cos-codex-bridge/scripts/install.mjs --root /absolute/project-parent`. The generated launcher points to that package folder, so keep it in place. Both packages were tested from clean npm installs through isolated demo installers and doctor runs. See the exact commands in [README](README.md#install).
